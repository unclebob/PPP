...
public class CompactDiscsController extends MediaController {
  ...
  protected ActionResult finishMediaCopiesRequest(ActionResult result) {
    setAttribute("compactDiscs", getApplication().getMediaGateway().findAllCDs());
  ...
  private void acceptCDs(ActionResult result, String id, String title, String artist, int ncopies) {
    Media cd = new Media(id, title, artist, Media.COMPACT_DISC);
    List<MediaCopy> copies = getApplication().getMediaGateway().addCopies(cd, ncopies);
  ...
  private void acceptNewCopiesForExistingCD(ActionResult result, String id, int numberOfNewCDs) {
    try {
      List<MediaCopy> mediaCopies = getLibrary().acceptCDs(id, numberOfNewCDs);
  ...
  private void appendSuccessfullyAddedCDsMessage(ActionResult result, String id, List<MediaCopy> mediaCopies) {
    MediaCopy copy = mediaCopies.get(0);
    Media disc = (Media) copy.getMedia();
