...
public class BooksController_LoanTest extends TestCase {
  ...
  public void testMatchPatronRendersPatronSelectorIfMoreThanOneMatch() throws Exception {
    ...
    bob.setId("" + registrar.getPatronGateway().getNextId());
    registrar.getPatronGateway().getPatronMap().put(bob.getId(), bob);
    bill.setId("" + registrar.getPatronGateway().getNextId());
    registrar.getPatronGateway().getPatronMap().put(bill.getId(), bill);
