...
import com.objectmentor.library.utils.TimeSource;
import com.objectmentor.library.web.controller.LibraryController;
import com.objectmentor.library.web.framework.ActionResult;

import java.util.*;

public class BooksController extends LibraryController {
  ...
  public ActionResult loanCopy() {
    ...
    if (isPost()) {
      ...
      if (patron == null) {
        render(IDENTIFY_PATRON);
        return ActionResult.makeErrorHandlerResult("TERRIBLE INTERNAL ERROR, patron is null!");
      } else if (copy == null) {
        renderTransaction(patron);
  ...
  public ActionResult returnCopy() {
    ...
    if (isPost()) {
      Patron patron = (Patron) getSession().getAttribute("patron");
      String copyId = getParameter("copyId");
      MediaCopy copy = bookGateway.findCopyById(copyId);
      if (copy == null) {
