...
<%@ page import="com.objectmentor.library.application.models.*" %>
...
<form action="<library:actionPath actionName="compactDiscs/manage"/>" method="post">
  ...
  <c:if test="${cdsExist}">
    ...
    <tr class="${evenodd}">
      <td width="100%">
        ...
        <%= copyId %>:&nbsp;<input type="checkbox" id='<%= "delete_"+disc0.getId()+"_"+copyId %>'
                                   name='<%= "delete_"+copyId %>'/>&nbsp;
        <% } %>
      </td>
      <td align="center">
      ...
      <td align="left">
        <input type="text" size="7" value="(number)" onfocus="select_value_on_first_focus(this);"
               name='<%= "newCopies_"+disc0.getId() %>'/>
      </td>
    </tr>
  ...
  <tr class="${evenodd}">
    <td colspan="3">
      <b>New:</b>&nbsp;<input type="text" name="id" value="(Bar code/other ID)"
                              onfocus="select_value_on_first_focus(this);"/>&nbsp;&dagger;&nbsp;<font
      color="red">*</font>
