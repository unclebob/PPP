...
public class InMemoryMediaGateway implements MediaGateway {
  ...
  public List findAllCopiesByIsbn(String isbn) {
    List copies = (List) this.mediaCopies.get(isbn);
    if (copies == null)
  ...
  public MediaCopy findAvailableCopy(String id) {
    List copies = findAllCopiesByIsbn(id);
  ...
  protected Media getFirstInList(List list) {
	...
	public List findAllCopiesByType(Class type) {
		List typeCollections = findAllCollectionsOfType(type);
		List allCopiesOfType = new ArrayList();
		for (Iterator i = typeCollections.iterator(); i.hasNext(); ) {
			allCopiesOfType.addAll((List) i.next());
		}
		return allCopiesOfType;
	}
}