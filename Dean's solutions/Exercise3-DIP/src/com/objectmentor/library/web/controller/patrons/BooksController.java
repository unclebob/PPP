...
import com.objectmentor.library.web.controller.Controller;
...
public class BooksController extends Controller {
  ...
  public ActionResult matchPatron() {
    ...
    Map patronMap = patronGateway.getPatronMap();
    Patron patron = (Patron) patronMap.get(pattern);
    if (patron == null) {
      Set patrons = findLike(pattern);
      if (!patrons.isEmpty()) {
  ...
  public Set findLike(String pattern) {
    ...
    for (Iterator i = getApplication().getPatronGateway().getPatronList().iterator(); i.hasNext();) {
  ...
  private List getLoanRecords(Patron patron) {
    ...
    List loanReceipts = getApplication().getMediaGateway().findAllLoanReceiptsFor(patron.getId());
  ...
  public ActionResult selectPatron() {
    ...
    Patron patron = (Patron) getApplication().getPatronGateway().getPatronMap().get(selectedPatron);
    renderTransaction(patron);
    return result;
  }
